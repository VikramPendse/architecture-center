...
2. For **Source server type**, Contoso selects **SQL Server on Azure Virtual Machines**.
    ...
    > [!NOTE]
    > Currently, Data Migration Assistant doesn't support assessment for migrating to an Azure SQL Database Managed Instance. As a workaround, Contoso uses SQL Server on an Azure VM as the supposed target for the assessment.Azure Data Migration Assistant have an option of "Azure SQL Database Managed Instance" when we choose "Project Type" - Assessment
